...
| A3 | *(fixed 2026-08-29, unconfirmed on hardware)* **No `CONTROL` read pump.** The vendor reads `0A10` every 700 ms; we never read anything. | Leading candidate for the 65 s disconnect. Also means rider button presses are invisible. | `core:link`'s new `ControlPump` polls every 700 ms with whole-frame dedup and bootstrap adoption, producing the `GENERAL` ack block. Toggleable from the debug menu (tap the version string ×7 on the Connect tab) for the B4 bisect. |
...
| A13 | *(fixed 2026-08-30)* **First real-device session found four separate defects**: (1) dark mode text was invisible - `BetterConnectTheme` never wrapped content in a themed `Surface`, so text painted onto the Activity's permanently-light `windowBackground`; (2) pressing Scan with Bluetooth off silently no-op'd (`BleScanner.start()`'s `adapter?.bluetoothLeScanner ?: return`) with no system enable-dialog anywhere in the app; (3) `Navigation` was only reachable through a button `Home` rendered conditionally on `ConnectionState.Ready`, contradicting the explicit "separate tabs like the official app" requirement; (4) `BleScanner`/`DeviceLocationSource` called permission-gated APIs with no runtime check, and `full`'s Nav3 back stack could restore straight past `Onboarding` after a permission was later revoked. | App was effectively unusable in dark mode, Bluetooth-off state looked identical to a broken app, the map required a completed bike connection to even view, and a revoked permission was a live crash path. | `BetterConnectTheme` now wraps content in `Surface(color = colorScheme.background)`; `values-night/themes.xml` added. `DevicesViewModel` seeds and gates on `bluetoothEnabled`, `DevicesScreen` shows a persistent "Turn on Bluetooth" card/button firing `ACTION_REQUEST_ENABLE`. `full` dropped Nav3 entirely for a persistent Connect/Navigate `NavigationBar` (see `.claude/rules/compose-ui.md`'s Navigation correction) - Navigate works with no connection, only `canStart` requires one. `BleScanner`/`DeviceLocationSource`/`ClusterService#startForegroundWithType` all gained `ContextCompat.checkSelfPermission` guards, and `FullApp()` re-checks `FullPermissions.allRequiredGranted` on every `ON_RESUME`, routing back to `Onboarding` if anything was revoked. |
